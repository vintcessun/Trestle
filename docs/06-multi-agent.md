...
**互斥收进单点分配。** 要卡的人向同一个仲裁者排队，daemon 天然把他们排成序。
不需要 CAS、不需要 TTL，也没有「锁悬着」的问题。

### 分工：插件查，host 挑

host 的仲裁者里**没有一个字是关于 GPU 的**——它只会「在一把锁里挑几个单位出来并记账」。
所有关于显卡的知识都在 `gpu` 插件里。分工的位置是这么定的：

> **互斥必须在一个点上**（所以挑在 host），
> **真实世界只有插件知道怎么问**（所以查在插件）。

```
job_start(target="gpu-4", gpus="auto:2")
   → plugins.call(gpu, gpu_acquire, {target:"gpu-4", count:2, purpose:"job train-v3"})
       → gpu.wasm: 查 nvidia-smi，得到一份快照
       → arbiter.acquire("gpu-4/gpu", 快照, 2, ...)
           → host 在一把锁里：快照 + 自己的账 → 原子选卡 → 记账
       → job 拿到 claim，设 CUDA_VISIBLE_DEVICES
```

这条边界还救了一个真实的 bug。上一版把两件事混在 host 里：分配时先取锁，再在锁里去
查 `nvidia-smi`，而那条查询路径会**再取一次同一把锁**。tokio 的 `Mutex` 不可重入，
所以第一次真的要卡就会永久挂死——它一直没被发现，因为没有测试真的走过那条路。
现在 host 侧那个函数**一行 I/O 都没有**，这一整类问题不可能再发生。

GPU 只是第一个用它的资源。下一个是许可证席位、独占的盘、还是某台机器上的某个端口，
host 都不需要改：写一个插件，声明 `arbitrate = ["<资源名>"]`，查好递进来就行。
...
job 把 claim 记在自己的登记里，任务结束（`job_list` / `job_wait` 看到 rc 文件）或者被
`job_stop` 停掉，都会把卡还回去——两条路径都要还，漏一条就是一张卡永远回不了池。

还有一层兜底：下一个人来要卡时，账上记着、真实世界里却全空、而且已经过了宽限期
（5 分钟）的占用会被顺手收掉。收尸放在这里做，是因为**恰好此刻有一份新鲜快照**，
也恰好此刻有人在等——单独开一个 reconcile 入口反而要自己去查真实世界，
那正是仲裁者不该做的事。

## 抢卡

`tasks.schedule` + 导出 `on-tick`：插件注册一个周期回调，每次醒来试一次
`gpu_acquire`——拿不到就继续睡，拿到就起任务再 `tasks.cancel`。
互斥由仲裁者保证，插件这边不需要任何锁逻辑。
